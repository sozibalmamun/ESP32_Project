...
{
	int len = send(SCI.sockID, BufferSend, Slen, 0);
	if (len < 0)
	{
...
void TCP_Client_Receive_Data_Task(void *pvParameters)
{
    ...
    // Set tcp keepalive option
     setsockopt(SCI.sockID, SOL_SOCKET, SO_KEEPALIVE, &keepAlive, sizeof(int));
    /// setsockopt(SCI.sockID, IPPROTO_TCP, TCP_KEEPIDLE, &keepIdle, sizeof(int));
     //setsockopt(SCI.sockID, IPPROTO_TCP, TCP_KEEPINTVL, &keepInterval, sizeof(int));
   //  setsockopt(SCI.sockID, IPPROTO_TCP, TCP_KEEPCNT, &keepCount, sizeof(int));

	while(1)
	{
		printf("sang:recv SCI.sockID:%d...\r\n",SCI.sockID);
		len = recv(SCI.sockID, (char*)SCI.ReceiveBuffer, sizeof(SCI.ReceiveBuffer)-1, 0);
		if (len < 0)
		{ 
		...
		else 
		{
			...
			//TCP_Client_SendData((char*)SCI.ReceiveBuffer,50);
...
void Tcp_Client_Connect_Task(void *pvParameters)
{
	...
	strcpy((char*)SCI.HostIPAddress,IPStr);
	SCI.Port=PortN;
	SCI.SocketClientConnectedState=false;
	struct sockaddr_in dest_addr;
	dest_addr.sin_addr.s_addr = inet_addr((char*)SCI.HostIPAddress);
	...
	xTaskCreatePinnedToCore(&TCP_Client_Receive_Data_Task, "TCP_Client_Receive_Data_Task", 4096, NULL, 2, NULL, 1);
	SCI.SocketClientTaskRuning=false;
	vTaskDelete(NULL);
...
static void TCP_ConT_task(void *arg) 
{
	printf("sang:TCP_ConT_task..\r\n");
  	xTaskCreate(WifiConnectStateMoniterTask, "WifiConnectStateMoniterTask", 4096, NULL,5, &Task_HandleIPMoniter); 
}
const esp_timer_create_args_t periodic_timer_Delay = {
	.callback = &TCP_ConT_task,
	.name = "periodic_Delay"};
    ...
    switch (event->event_id)
    {
		...
		case SYSTEM_EVENT_STA_GOT_IP:
			...
			printf("sang:Got IP:%08X\r\n",WifiInfosang.CurrentStationIP);
   			...
   			//ESP_ERROR_CHECK(esp_timer_create(&periodic_timer_Delay, &Delay_timer));
   			//ESP_ERROR_CHECK(esp_timer_start_periodic(Delay_timer, 1000 * 1000));
			//ESP_ERROR_CHECK(esp_timer_start_once(Delay_timer, 3000 * 1000));
			xTaskCreate(WifiConnectStateMoniterTask, "WifiConnectStateMoniterTask", 4096, NULL,5, &Task_HandleIPMoniter); 

...
void lv_img_set_src(lv_obj_t * obj, const void * src)
{
   ...
   // LV_LOG_USER("src_type:%d\r\n",src_type);
//#if LV_USE_LOG && LV_LOG_LEVEL >= LV_LOG_LEVEL_INFO
#if LV_USE_LOG && LV_LOG_LEVEL >= LV_LOG_LEVEL_TRACE
    switch(src_type) {
...
//	LV_LOG_USER("img->w:%d,img->h:%d,img->cf:%d\r\n",img->w,img->h,img->cf);
...
static void draw_img(lv_event_t * e)
{
	...
	{
			//LV_LOG_USER("sang:LV_EVENT_COVER_CHECK\r\n");
		...
		//LV_LOG_USER("sang:LV_EVENT_DRAW_MAIN\r\n");
        ...
        if(code == LV_EVENT_DRAW_MAIN) {
            ...
            if(img->src_type == LV_IMG_SRC_FILE || img->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                LV_LOG_WARN("image source is NULL");
				 LV_LOG_USER("sang:image source is NULL\r\n");
            }
            else {
                /*Trigger the error handler of image draw*/
                LV_LOG_WARN("image source type is unknown");
				LV_LOG_USER("sang:image source type is unknown\r\n");

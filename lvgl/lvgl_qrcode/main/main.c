...
static void lvgl_task(void *arg) // GUI任务
{
   ...
   lv_color_t bg_color = lv_palette_lighten(0xff, 5);
    lv_color_t fg_color = lv_palette_darken(LV_PALETTE_YELLOW, 4);

    lv_obj_t * myqrcode = lv_qrcode_create(lv_scr_act());
    lv_qrcode_set_size(myqrcode, 160);
    lv_qrcode_set_dark_color(myqrcode, fg_color);
    lv_qrcode_set_light_color(myqrcode, bg_color);

    /*Set data*/
    const char * data = "禾木科技";
    lv_qrcode_update(myqrcode, data, strlen(data));
    lv_obj_center(myqrcode);

    /*Add a border with bg_color*/
    lv_obj_set_style_border_color(myqrcode, bg_color, 0);
    lv_obj_set_style_border_width(myqrcode, 6, 0);

...
static void lvgl_task(void *arg) // GUI任务
{
   ...
   lv_obj_t * btnm1 = lv_btnmatrix_create(lv_scr_act());				// 在主屏幕上创建一个矩阵按钮
     lv_btnmatrix_set_map(btnm1, btnm_map);									// 设置按钮数组
     lv_btnmatrix_set_btn_width(btnm1, 15, 2);								// 使=的宽度是AC的两倍
     lv_btnmatrix_set_btn_ctrl(btnm1, 15, LV_BTNMATRIX_CTRL_CHECKABLE);		// =单击可变
     
     lv_btnmatrix_set_btn_ctrl(btnm1, 16, LV_BTNMATRIX_CTRL_CHECKED);	// AC为Check状态
     lv_obj_align(btnm1, LV_ALIGN_CENTER, 0, 0);						// 对齐到屏幕中心

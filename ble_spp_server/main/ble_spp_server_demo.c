    ...
    switch (event) {
    	...
    	case ESP_GATTS_READ_EVT:
            ...
            if(res == SPP_IDX_SPP_STATUS_VAL){
                ...
                ESP_LOGI(GATTS_TABLE_TAG, "sozib : handle = %d\n", res);
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if (param->write.handle == spp_handle_table[SPP_IDX_SPP_DATA_RECV_VAL]) {//SPP_IDX_SPP_DATA_RECV_VAL
                // Data written to SPP_COMMAND_RECEIVE characteristic
                // Process the received command
                uint8_t *recv_data = param->write.value;
                uint16_t recv_len = param->write.len;
                
                // Handle the received command/data here
                // Example: Print received data to console
                ESP_LOGI(GATTS_TABLE_TAG, "Received command: %.*s", recv_len, recv_data);
                
                // Optionally, send a response or execute commands based on the received data
            }

...
static void websocket_app_start(void)
{
    ...
    // Subscribe to /topic/cloud
    const char *subscribe_command = "{\"command\":\"subscribe\",\"topic\":\"/topic/cloud\"}";
    esp_err_t ret = esp_websocket_client_send_text(client, subscribe_command, strlen(subscribe_command), portMAX_DELAY);
    if (ret < 0) {
        ESP_LOGE(TAG, "Failed to send subscribe command: %s", esp_err_to_name(ret));

...
static void lvgl_task(void *arg) // GUI任务
{
   ...
   lv_label_set_text(label,"禾木科技");//中文内容
   lv_obj_align(label, LV_ALIGN_CENTER, 0, 0); // 位置

   lv_style_t mystyle;
   lv_style_init(&mystyle);
   lv_style_set_text_font(&mystyle,&myfont); //引用字库
   lv_obj_add_style(label, &mystyle, 0);
   while (1)
   {
